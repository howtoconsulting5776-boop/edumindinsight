...
function deidentify(text: string): string {
  ...
  // 이름 + 씨/님/군/양 패턴만 제한적으로 비식별화 (일반 명사 오치환 방지)
  result = result.replace(
    /(?<![가-힣])([가-힣]{2,4})\s*(씨|군|양)(?![가-힣])/g,
    (_, _name, suffix) => suffix === "씨" ? "학부모님" : "해당 학생"
  )
...
## 중요 출력 규칙
- 스크립트와 hiddenIntent에 절대로 "OO", "○○", "XX" 같은 플레이스홀더를 사용하지 마세요.
- 학생 이름을 모를 경우 "해당 학생", 학부모는 "학부모님"으로 표현하세요.
- 모든 스크립트는 이름 자리를 채운 완성된 문장으로 작성하세요.`
  }
...
[원칙형 대응]
"말씀 주신 부분, 저도 100% 공감합니다. 소통이 부족했던 건 사실입니다. 다만 학습 측면에서는 지난 8주간 해당 학생의 단원 이해도가 꾸준히 향상되고 있습니다. 구체적으로는 이번 달 2회 단원 평가에서 각각 71점, 84점을 기록했고, 오답 유형도 계산 실수 중심으로 좁혀졌습니다. 이 데이터와 함께 앞으로 2개월 학습 로드맵을 이번 주 내로 서면으로 공유드리겠습니다."
