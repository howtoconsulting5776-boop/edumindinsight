...
-- ── 4. knowledge_base 테이블 (없으면 생성, 있으면 누락 컬럼만 추가) ─────────
...
-- 기존 테이블에 누락 컬럼 안전 추가
ALTER TABLE knowledge_base ADD COLUMN IF NOT EXISTS academy_id UUID REFERENCES academies(id) ON DELETE CASCADE;
ALTER TABLE knowledge_base ADD COLUMN IF NOT EXISTS tags       TEXT[] DEFAULT '{}';
ALTER TABLE knowledge_base ADD COLUMN IF NOT EXISTS situation  TEXT;
ALTER TABLE knowledge_base ADD COLUMN IF NOT EXISTS response   TEXT;
ALTER TABLE knowledge_base ADD COLUMN IF NOT EXISTS outcome    TEXT;
